...
public class Filters {
    public static void main(String[] args) throws IOException {
        TestTableHelper.execute(new TestTableHelper.Executor() {
            ...
            public void execute(Connection connection, Table table) throws IOException {
                cleanAndInsert("row", table);
                cleanAndInsert("row-", table);
                scanWithPrefixFilter("row-1", table);
                scanWithPrefixFilter("row-2", table);
    ...
    static void cleanAndInsert(String rowKeyPrefix, Table table) throws IOException {
        ...
        for (int i = 0 ; i < num; i++) {
            ...
            put.addColumn(Bytes.toBytes("cf"), Bytes.toBytes("qual1"), Bytes.toBytes(String.format("val%d", i)));
            puts.add(put);
        }
